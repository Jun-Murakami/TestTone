#!/bin/bash
# Emscripten で TestTone DSP を WASM にビルドする。
...
mkdir -p "${DIST_DIR}"
cp -f "${BUILD_DIR}/dist/testtone_dsp.wasm" "${DIST_DIR}/"

# WebUI の public-web/wasm/ にもコピー（開発・本番ともここから配信される）
mkdir -p "${WEB_PUBLIC_WASM}"
cp -f "${BUILD_DIR}/dist/testtone_dsp.wasm" "${WEB_PUBLIC_WASM}/"

echo ""
echo "Build complete."
echo "  WASM: ${DIST_DIR}/testtone_dsp.wasm"
