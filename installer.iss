...
OutputBaseFilename=TestTone_{#MyAppVersion}_Windows_Setup
...
Source: "build\plugin\TestTone_artefacts\Release\Standalone\TestTone.exe"; DestDir: "{code:GetStandalonePath}"; Flags: ignoreversion; Components: standalone
Source: "build\plugin\TestTone_artefacts\Release\Standalone\*.dll"; DestDir: "{code:GetStandalonePath}"; Flags: ignoreversion skipifsourcedoesntexist; Components: standalone

; VST3 Plugin
Source: "build\plugin\TestTone_artefacts\Release\VST3\TestTone.vst3\*"; DestDir: "{code:GetVST3Path}\TestTone.vst3"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: vst3

; CLAP Plugin — 単一ファイル（.clap = DLL に拡張子を付けたもの）
Source: "releases\{#MyAppVersion}\Windows\TestTone.clap"; DestDir: "{code:GetCLAPPath}"; Flags: ignoreversion; Components: clap

; AAX Plugin — バンドル全体を丸ごとコピー（入れ子は PrepareToInstall で事前削除により防止）
Source: "releases\{#MyAppVersion}\Windows\TestTone.aaxplugin\*"; DestDir: "{code:GetAAXPath}\TestTone.aaxplugin"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: aax
...
Type: filesandordirs; Name: "{localappdata}\TestTone"
Type: filesandordirs; Name: "{userappdata}\TestTone"

[Registry]
; Register VST3 plugin location
Root: HKLM64; Subkey: "Software\VST3"; ValueType: string; ValueName: "TestTone"; ValueData: "{code:GetVST3Path}\TestTone.vst3"; Flags: uninsdeletevalue; Components: vst3

; Register CLAP plugin location（CLAP 規約に厳密なレジストリ要件は無いが、参照用に同じパターンで残す）
Root: HKLM64; Subkey: "Software\CLAP"; ValueType: string; ValueName: "TestTone"; ValueData: "{code:GetCLAPPath}\TestTone.clap"; Flags: uninsdeletevalue; Components: clap

; Register AAX plugin location
Root: HKLM64; Subkey: "Software\Avid\ProTools\AAX"; ValueType: string; ValueName: "TestTone"; ValueData: "{code:GetAAXPath}\TestTone.aaxplugin"; Flags: uninsdeletevalue; Components: aax
...
begin
  ...
  PathSelectionPage.Values[0] := ExpandConstant('{autopf}\TestTone');
  ...
  begin
    ...
    begin
      if MsgBox('Microsoft Visual C++ Redistributables are required but not installed.' + #13#10 + #13#10 +
                ...
                '(Required for TestTone to run properly)', mbConfirmation, MB_YESNO) = IDYES then
      begin
        DownloadPage.Clear;
        ...
        try
          ...
          MsgBox('Failed to download Visual C++ Redistributables.' + #13#10 +
                 'TestTone may not work properly without them.' + #13#10 + #13#10 +
      ...
      begin
        MsgBox('Warning: TestTone may not work without Visual C++ Redistributables.' + #13#10 + #13#10 +
...
begin
  ...
  begin
    ...
    TestFile := TargetPath + '\TestTone.vst3\Contents\x86_64-win\TestTone.vst3';
    ...
    begin
      ...
      begin
        ...
        if MsgBox('The VST3 plugin appears to be in use by a DAW.' + #13#10 + #13#10 +
                  'Please close any DAW that might be using TestTone and try again.' + #13#10 + #13#10 +
  ...
  begin
    ...
    TestFile := TargetPath + '\TestTone.clap';
    if FileExists(TestFile) then
    begin
      FileLocked := not DeleteFile(TestFile);
      if FileLocked then
  ...
  begin
    ...
    if DirExists(TargetPath + '\TestTone.aaxplugin') then
    begin
      Log('Removing existing AAX bundle to avoid nested installation');
      if not DelTree(TargetPath + '\TestTone.aaxplugin', True, True, True) then
      begin
  ...
  begin
    ...
    if StandalonePath <> '' then
      ...
      TargetPath := ExpandConstant('{autopf}\TestTone');

    TestFile := TargetPath + '\TestTone.exe';
    if FileExists(TestFile) then
    begin
      FileLocked := not DeleteFile(TestFile);
      if FileLocked then
      begin
        ...
        if MsgBox('TestTone standalone application appears to be running.' + #13#10 + #13#10 +
                  'Please close it and try again.' + #13#10 + #13#10 +
                  'Continue anyway? (Installation may fail)', mbConfirmation, MB_YESNO) = IDNO then
        begin
          Result := 'TestTone.exe is running. Please close it and try again.';
          Exit;
        end;
      end;
    end;
...
begin
  if StandalonePath = '' then
    Result := ExpandConstant('{autopf}\TestTone')
  else
...
begin
  ...
  DelTree(ExpandConstant('{localappdata}\TestTone'), True, True, True);
  DelTree(ExpandConstant('{userappdata}\TestTone'), True, True, True);
end;
