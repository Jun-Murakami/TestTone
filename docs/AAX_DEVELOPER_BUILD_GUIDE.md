...
3. **AAXプラグインの署名**
   ...
   wraptool.exe sign --verbose \
     ...
     --signid "Developer" \
     --in "TestTone.aaxplugin" \
     --out "TestTone_signed.aaxplugin"
...
1. **依存ライブラリの確認**
   ...
   depends.exe TestTone.aaxplugin

   # macOS
   otool -L TestTone.aaxplugin/Contents/MacOS/TestTone
   ```
...
2. **ログ機能の実装**
   ...
   // AAXプラグイン内でのログ
   #ifdef DEBUG
   AAX_TRACE(AAX_TRACE_PRIORITY_NORMAL, "TestTone: %s", message);
   #endif
   ```
