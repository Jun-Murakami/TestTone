...
# TestTone macOS Release Build Script (zsh)
...
echo_header "TestTone ${VERSION} Build Script (macOS zsh)"
...
EXISTING_ARTIFACTS_DIR="${BUILD_DIR}/plugin/TestTone_artefacts/${CONFIGURATION}"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target TestTone_VST3 TestTone_AU TestTone_Standalone TestTone_CLAP TestTone_AAX
else
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target TestTone_VST3 TestTone_AU TestTone_Standalone TestTone_CLAP
fi
echo_success "Plugin build completed"

# Artifact paths
ARTIFACTS_DIR="${BUILD_DIR}/plugin/TestTone_artefacts/${CONFIGURATION}"

# Verify Universal Binary
echo_step "Checking architecture of built plugins..."
if [[ -f "${ARTIFACTS_DIR}/VST3/TestTone.vst3/Contents/MacOS/TestTone" ]]; then
    echo "  VST3:"
    lipo -info "${ARTIFACTS_DIR}/VST3/TestTone.vst3/Contents/MacOS/TestTone"
fi
if [[ -f "${ARTIFACTS_DIR}/AU/TestTone.component/Contents/MacOS/TestTone" ]]; then
    echo "  AU:"
    lipo -info "${ARTIFACTS_DIR}/AU/TestTone.component/Contents/MacOS/TestTone"
fi
if [[ -f "${ARTIFACTS_DIR}/Standalone/TestTone.app/Contents/MacOS/TestTone" ]]; then
    echo "  Standalone:"
    lipo -info "${ARTIFACTS_DIR}/Standalone/TestTone.app/Contents/MacOS/TestTone"
fi
if [[ -f "${ARTIFACTS_DIR}/CLAP/TestTone.clap/Contents/MacOS/TestTone" ]]; then
    echo "  CLAP:"
    lipo -info "${ARTIFACTS_DIR}/CLAP/TestTone.clap/Contents/MacOS/TestTone"
fi
if [[ ${BUILD_AAX} -eq 1 ]] && [[ -f "${ARTIFACTS_DIR}/AAX/TestTone.aaxplugin/Contents/MacOS/TestTone" ]]; then
    echo "  AAX:"
    lipo -info "${ARTIFACTS_DIR}/AAX/TestTone.aaxplugin/Contents/MacOS/TestTone"
fi
echo_success "Architecture verification completed"
SRC_VST3="${ARTIFACTS_DIR}/VST3/TestTone.vst3"
SRC_AU="${ARTIFACTS_DIR}/AU/TestTone.component"
SRC_APP="${ARTIFACTS_DIR}/Standalone/TestTone.app"
SRC_CLAP="${ARTIFACTS_DIR}/CLAP/TestTone.clap"
...
# AAX check
if [[ ${BUILD_AAX} -eq 1 ]]; then
    SRC_AAX="${ARTIFACTS_DIR}/AAX/TestTone.aaxplugin"
    if [[ ! -d "${SRC_AAX}" ]]; then echo_error "AAX not found: ${SRC_AAX}"; exit 1; fi
fi

echo_step "Collecting artifacts..."
DEST_VST3="${OUTPUT_DIR}/TestTone.vst3"
DEST_AU="${OUTPUT_DIR}/TestTone.component"
DEST_APP="${OUTPUT_DIR}/TestTone.app"
DEST_CLAP="${OUTPUT_DIR}/TestTone.clap"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    DEST_AAX="${OUTPUT_DIR}/TestTone.aaxplugin"
    rm -rf "${DEST_AAX}"
...
if [[ -f "${DEST_VST3}/Contents/MacOS/TestTone" ]]; then
    INFO="$(lipo -info "${DEST_VST3}/Contents/MacOS/TestTone" 2>/dev/null || true)"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
   ...
   Copy the entire TestTone.aaxplugin folder to the following location:
...
TestTone ${VERSION} - macOS Installation Guide
====================================================
...
2. For VST3 Plugin:
   Copy the entire TestTone.vst3 folder to the following location:
   ~/Library/Audio/Plug-Ins/VST3/

3. For Audio Unit (AU):
   Copy the entire TestTone.component folder to the following location:
   ~/Library/Audio/Plug-Ins/Components/

4. For Standalone Application:
   Copy TestTone.app to any preferred location, for example:
   /Applications/ or your Desktop.

5. For CLAP Plugin:
   Copy the entire TestTone.clap folder to the following location:
...
PKG_ID_BASE="${PKG_ID_BASE:-com.bucketrelay.testtone}"
...
PKG_VST3="${PKG_WORK_DIR}/TestTone_VST3.pkg"
pkgbuild \
    --root "${PKGROOT_VST3}" \
...
PKG_AU="${PKG_WORK_DIR}/TestTone_AU.pkg"
pkgbuild \
    --root "${PKGROOT_AU}" \
...
PKG_APP="${PKG_WORK_DIR}/TestTone_Standalone.pkg"
pkgbuild \
    --component "${DEST_APP}" \
...
PKG_CLAP="${PKG_WORK_DIR}/TestTone_CLAP.pkg"
pkgbuild \
    --root "${PKGROOT_CLAP}" \
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ...
    PKG_AAX="${PKG_WORK_DIR}/TestTone_AAX.pkg"
    pkgbuild \
        --root "${PKGROOT_AAX}" \
...
fi
{
    ...
    echo "  <title>TestTone ${VERSION}</title>"
    ...
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.vst3\">TestTone_VST3.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.au\">TestTone_AU.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.app\">TestTone_Standalone.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.clap\">TestTone_CLAP.pkg</pkg-ref>"
    if [[ ${BUILD_AAX} -eq 1 ]]; then
        echo "  <pkg-ref id=\"${PKG_ID_BASE}.aax\">TestTone_AAX.pkg</pkg-ref>"
    fi
...
PRODUCT_PKG_PATH="${OUTPUT_DIR}/../TestTone_${VERSION}_macOS.pkg"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ZIP_NAME="TestTone_${VERSION}_macOS_VST3_AU_AAX_CLAP_Standalone.zip"
else
    ZIP_NAME="TestTone_${VERSION}_macOS_VST3_AU_CLAP_Standalone.zip"
fi
