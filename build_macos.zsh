...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target TestTone_VST3 TestTone_AU TestTone_Standalone TestTone_AAX
else
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target TestTone_VST3 TestTone_AU TestTone_Standalone
fi
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ZIP_NAME="TestTone_${VERSION}_macOS_VST3_AU_AAX_Standalone.zip"
else
    ZIP_NAME="TestTone_${VERSION}_macOS_VST3_AU_Standalone.zip"
fi
