# TestTone Windows Release Build Script
...
# Start build process
Write-Header "TestTone $Version Build Script"
...
cmake --build . --config $Configuration --target TestTone_VST3
if ($LASTEXITCODE -ne 0) {
...
cmake --build . --config $Configuration --target TestTone_Standalone
if ($LASTEXITCODE -ne 0) {
...
cmake --build . --config $Configuration --target TestTone_CLAP
if ($LASTEXITCODE -ne 0) {
...
if ($BuildAAX) {
    ...
    cmake --build . --config $Configuration --target TestTone_AAX
    if ($LASTEXITCODE -ne 0) {
...
$SourceVST3 = "$BuildDir\plugin\TestTone_artefacts\$Configuration\VST3\TestTone.vst3"
$DestVST3 = "$OutputDir\Windows\TestTone.vst3"
...
$SourceStandalone = "$BuildDir\plugin\TestTone_artefacts\$Configuration\Standalone\TestTone.exe"
$DestStandalone = "$OutputDir\Windows\TestTone.exe"
...
$SourceCLAP = "$BuildDir\plugin\TestTone_artefacts\$Configuration\CLAP\TestTone.clap"
$DestCLAP = "$OutputDir\Windows\TestTone.clap"
...
if ($BuildAAX) {
    ...
    $SourceAAX = "$BuildDir\plugin\TestTone_artefacts\$Configuration\AAX\TestTone.aaxplugin"
    $DestAAX = "$OutputDir\Windows\TestTone.aaxplugin"
    
    if (Test-Path $SourceAAX) {
        # 既存の AAX バンドルがある場合は削除してからコピーする。
        # これを行わないと、Copy-Item が入れ子の
        # TestTone.aaxplugin\TestTone.aaxplugin\... を作ってしまう。
        ...
        $AAXBinary = Join-Path $DestAAX "Contents\x64\TestTone.aaxplugin"
...
$ReadmeContent = @"
TestTone $Version - Windows Installation Guide
====================================================
...
3. For Standalone Application:
   Copy TestTone.exe to any preferred location, for example:
   C:\Program Files\TestTone\ or your Desktop.

4. For CLAP Plugin:
   Copy TestTone.clap to the following location:
...
if (Test-Path $InnoSetupPath) {
    ...
    if (Test-Path $InstallerScript) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $InstallerPath = "$OutputDir\TestTone_${Version}_Windows_Setup.exe"
...
if ($BuildAAX) {
    $ZipName = "TestTone_${Version}_Windows_VST3_AAX_CLAP_Standalone.zip"
} else {
    $ZipName = "TestTone_${Version}_Windows_VST3_CLAP_Standalone.zip"
}
...
Write-Host "[✓] TestTone.vst3 (with embedded WebUI)" -ForegroundColor Green
Write-Host "[✓] TestTone.exe (Standalone application)" -ForegroundColor Green
Write-Host "[✓] TestTone.clap (CLAP plugin)" -ForegroundColor Green
if ($BuildAAX) {
    Write-Host "[✓] TestTone.aaxplugin ($AAXSigningStatusSummary)" -ForegroundColor Green
}
...
if (Test-Path "$OutputDir\TestTone_${Version}_Windows_Setup.exe") {
